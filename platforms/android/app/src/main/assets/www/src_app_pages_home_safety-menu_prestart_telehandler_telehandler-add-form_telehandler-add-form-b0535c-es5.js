        ...
        _createClass(TelehandlerAddFormPage, [{
          ...
          value: function pickImage(sourceType, tabName) {
            ...
            this.camera.getPicture(options).then(function (imageData) {
              ...
              _this.sharedService.getBase64(imageData).then(function (data) {
                image = data;
              });
              ...
              if (tabName == 'DataPlateImage') {
                console.log('fourth', image); // this.dataPlateImage = image;

                _this.teleHandlerForm.controls['data_plate_image'].setValue(image);

                console.log('DataPlateImage', _this.teleHandlerForm.value['data_plate_image']);
              }
                      ...
                      return this.actionSheetController.create({
                        ...
                        buttons: [{
                          ...
                          handler: function handler() {
                            _this2.pickImage(_this2.camera.PictureSourceType.PHOTOLIBRARY, tabName);
                          }
                        }, {
                          text: 'Use Camera',
                          handler: function handler() {
                            _this2.pickImage(_this2.camera.PictureSourceType.CAMERA, tabName);
                          }
                        }, {
                          text: 'Cancel',
                          role: 'cancel'
          ...
          key: "onDataPlateImage",
          value: function onDataPlateImage(event, type) {
                ...
                while (1) {
                  switch (_context4.prev = _context4.next) {
                    case 0:
                      ...
                      console.log('event 2', event.target.files); // // console.log('event 3', event.target.files[0]);

                      console.log('type', type);

                      if (type == 1) {
                        this.mobileUploads('DataPlateImage'); // await this.sharedService.mobileUploadImages().then(result => {
                        ...
                        this.sharedService.getBase64(event.target.files[0]).then(function (data) {
                          _this3.teleHandlerForm.controls['data_plate_image'].setValue(data);

                          _this3.loadingService.dismissLoading();
                        })["catch"](function (error) {
                          console.log('error', error); // this.loadingService.dismissLoading();
                        });
                      }
