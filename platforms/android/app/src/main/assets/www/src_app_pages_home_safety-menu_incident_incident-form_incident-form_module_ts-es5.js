      ...
      var _IncidentFormPage = /*#__PURE__*/function () {
        ...
        _createClass(IncidentFormPage, [{
          ...
          value: function onPhotoGraphy(event, type) {
            ...
            if (type == 1) {// let data = this.mobileUploadImages();
              ...
              var _loop = function _loop(i) {
                _this7.sharedService.getBase64(event.target.files[i]).then(function (data) {
                  _this7.photoGraphyObject[i] = data;

                  if (_this7.photoGraphy.length <= 0) {
                    _this7.photoGraphy.push(data);
                  } else {
                    _this7.photoGraphy.unshift(data);
                  }
            ...
            for (var i = 0; i < event.target.files.length; i++) {
              this.sharedService.getBase64(event.target.files[i]).then(function (data) {
                ...
                if (_this8.alcohalImages.length <= 0) {
                  _this8.alcohalImages.push(data);
                } else {
                  _this8.alcohalImages.unshift(data);
                }
              })["catch"](function (error) {
                console.log('error', error);
              });
          ...
          value: function onDrugTestImage(event) {
            ...
            for (var i = 0; i < event.target.files.length; i++) {
              this.sharedService.getBase64(event.target.files[i]).then(function (data) {
                ...
                if (_this9.drugTestImages.length <= 0) {
                  _this9.drugTestImages.push(data);
                } else {
                  _this9.drugTestImages.unshift(data);
                }
              })["catch"](function (error) {
                console.log('error', error);
              });
            ...
            for (var i = 0; i < event.target.files.length; i++) {
              this.sharedService.getBase64(event.target.files[i]).then(function (data) {
                ...
                if (_this10.alterDutyImages.length <= 0) {
                  _this10.alterDutyImages.push(data);
                } else {
                  _this10.alterDutyImages.unshift(data);
                }
              })["catch"](function (error) {
                console.log('error', error);
              });
          ...
          value: function onChemicalImageSelect(event) {
            ...
            for (var i = 0; i < event.target.files.length; i++) {
              this.sharedService.getBase64(event.target.files[i]).then(function (data) {
                ...
                if (_this11.chemicalImages.length <= 0) {
                  _this11.chemicalImages.push(data);
                } else {
                  _this11.chemicalImages.unshift(data);
                }
              })["catch"](function (error) {
                console.log('error', error);
              });
          ...
          value: function onSelectDamageImage(event) {
            ...
            for (var i = 0; i < event.target.files.length; i++) {
              this.sharedService.getBase64(event.target.files[i]).then(function (data) {
                ...
                if (_this12.damageImages.length <= 0) {
                  _this12.damageImages.push(data);
                } else {
                  _this12.damageImages.unshift(data);
                }
              })["catch"](function (error) {
                console.log('error', error);
              });
                    ...
                    case 2:
                      ...
                      modal.onDidDismiss().then(function (res) {
                        ...
                        if ((_a = res === null || res === void 0 ? void 0 : res.data) === null || _a === void 0 ? void 0 : _a.full_name) {
                          _this13.managerList = res.data.full_name;

                          _this13.classificationForm.controls.classification_manager.setValue(res.data.full_name);
                        }
                      });
                      _context.next = 6;
          ...
          value: function onSubmit(val) {
            ...
            console.log('classificationForm', this.classificationForm.value); // console.log('injuryForm', this.injuryForm.value);
            // console.log('enviornmentForm', this.enviornmentForm.value);

            console.log('reputationDesForm', this.reputationDesForm.value); // console.log('securityForm', this.securityForm.value);
            ...
            if (val == 'submit' && validation || val == 'incomplete_submit' && validation == false) {
              ...
              this.globalService.postData1(url, fd).subscribe(function (res) {
                ...
                if (res.status) {
                  _this14.globalService.presentToast(res.message);

                  _this14.nav.navigateRoot("form-list");
                } else {
                  _this14.globalService.presentToast(res.message);
          ...
          value: function onInput(evt, val) {
            ...
            this.tabList.find(function (ele) {
              if (val == 'Incident' && _this15.incidentForm.valid) {
                if (ele.val != 'Classification') {
                  ele.tabDisable = false;
                }
              } else if (val == 'Incident Description' && _this15.incidentDesForm.valid) {
                ele.tabDisable = false;
              } else {
                if (val == 'Incident Description' && _this15.incidentDesForm.invalid && ele.val == 'Classification') {
                  ele.tabDisable = true;
                  _this15.selectedTabList = [];

                  _this15.classificationForm.reset();

                  _this15.injuryForm.reset();

                  _this15.enviornmentForm.reset();

                  _this15.reputationDesForm.reset();

                  _this15.securityForm.reset();

                  _this15.assetDescriptionForm.reset();

                  _this15.reportForm.reset();
                }
              }
            });
