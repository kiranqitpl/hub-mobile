  ...
  (self["webpackChunkhub"] = self["webpackChunkhub"] || []).push([["src_app_pages_incident-form_incident-form_module_ts"], {
    ...
    function _(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
      ...
      var _IncidentFormPage = /*#__PURE__*/function () {
        function IncidentFormPage(globalService, sharedService, modal, actionSheetController, camera, fb, nav) {
          ...
          this.itemsArray = [1];
          this.reputationWitnessList = []; //-------------------------------------- Forms ----------------------------------------------------//
          //-------------------------------------- Tab ----------------------------------------------------//
        ...
        _createClass(IncidentFormPage, [{
          ...
          value: function onSelectTabItem(i) {
            ...
            this.selectedTabItem = i; // console.log('  this.selectedTabItem', this.selectedTabItem);
          }
        }, {
                      ...
                      return this.actionSheetController.create({
                        ...
                        buttons: [{
                          ...
                          handler: function handler() {
                            _this5.pickImage(_this5.camera.PictureSourceType.PHOTOLIBRARY, e);
                          }
                        }, {
                          text: 'Use Camera',
                          handler: function handler() {
                            _this5.pickImage(_this5.camera.PictureSourceType.CAMERA, e);
                          }
                        }, {
                          text: 'Cancel',
                          role: 'cancel'
          ...
          value: function pickImage(sourceType, e) {
            ...
            this.camera.getPicture(options).then(function (imageData) {
              ...
              var image = 'data:image/jpeg;base64,' + imageData; // if (e == 1) {
                    ...
                    case 2:
                      ...
                      modal.onDidDismiss().then(function (res) {
                        ...
                        if ((_a = res === null || res === void 0 ? void 0 : res.data) === null || _a === void 0 ? void 0 : _a.full_name) {
                          _this6.managerList = res.data.full_name;

                          _this6.classificationForm.controls.classification_manager.setValue(res.data.full_name);
                        }
                      });
                      _context2.next = 6;
          ...
          value: function onSubmit(val) {
            ...
            if (val == 'submit' && validation || val == 'incomplete_submit' && validation == false) {
              ...
              fd.append("return_to_alternate_duties_image", this.incidentDesForm.value['return_to_alternate_duties_image']); //----------------------------------- Incident Description -----------------------------------//
              //----------------------------------- Classification -----------------------------------//
              ...
              fd.append("person_details", JSON.stringify(this.injuryForm.value['person_details'])); // fd.append("part_of_body_injured_occured", this.injuryForm.value['part_of_body_injured_occured']);
              //-------------------------------------------Injury ----------------------------------//
              //-------------------------------------------Enviornment ----------------------------------//
              ...
              fd.append("emergency_spill_kit_used", this.enviornmentForm.value['emergency_spill_kit_used']); //-------------------------------------------Enviornment ----------------------------------//
              //-------------------------------------------Reputation ----------------------------------// 
              ...
              fd.append("extent_damage_image", this.assetDescriptionForm.value['extent_damage_image']); //-------------------------------------------Asset ----------------------------------//
              //-------------------------------------------Report ----------------------------------//
              ...
              this.globalService.postData1(url, fd).subscribe(function (res) {
                ...
                if (res.status) {
                  _this7.globalService.presentToast(res.message);
                ...
                _this7.globalService.dismissLoading();
              });
            }
          }
        }, {
          ...
          value: function onClassificationChange(event) {
            ...
            this.selectedTabList = formArray.value; // console.log('formArray.value', this.selectedTabList);
          }
        }, {
          key: "onInitialInjuryChange",
          value: function onInitialInjuryChange(event, personalDetailIndex, injueyListIndex) {// for (var j = 0; j <= this.injuryForm.value['person_details'].length; j++) {
            //   if (event.target.checked) {
          ...
          value: function onInput(val) {
            ...
            this.tabList.find(function (ele) {
              if (val == 'Incident' && _this8.incidentForm.valid) {
                if (ele.val != 'Classification') {
                  ele.tabDisable = false;
                }
              } else if (val == 'Incident Description' && _this8.incidentDesForm.valid) {
                ele.tabDisable = false;
              } else {
                if (val == 'Incident Description' && _this8.incidentDesForm.invalid && ele.val == 'Classification') {
                  ele.tabDisable = true;
                  _this8.selectedTabList = [];

                  _this8.classificationForm.reset();

                  _this8.injuryForm.reset();

                  _this8.enviornmentForm.reset();

                  _this8.reputationDesForm.reset();

                  _this8.securityForm.reset();

                  _this8.assetDescriptionForm.reset();

                  _this8.reportForm.reset();
                }
              }
            });
